...
* **vhost** — `docs/campagna-2026-09-13/evidenze/VHOST_STAGING_EVIDENCE_2026-09-10.md`. Read
...
* **public tunnels** (`bore local`) —
  `docs/campagna-2026-09-13/evidenze/PUBLIC_STAGING_EVIDENCE_2026-09-11.md`, with the Italian
  write-up in `docs/campagna-2026-09-13/public/final_public_perf_review.md`. Its harness lives
...
| `secret_leak_hunt.sh` | any Linux host, **no root, no deployment** | `all` (default) \| `churn` \| `churn-relay` \| `churn-direct` \| `stall` \| `stall-direct` \| `reap` \| `upgrade` \| `upgrade-late` \| `udpbuf`. The SECRET-tunnel resource gate: descriptors, resident memory, wedges, reaping and socket buffers, for a server + `bore local --tcp-secret-id` + `bore proxy` triple inside a rootless netns (`unshare -rn`). Every churn verdict is a RATE — four identical phases of 200 connections, the LAST phase asserted — because a single before/after pair cannot tell a leak from allocator warm-up (it accused the client of 16 KiB per connection when the truth was an arena that stopped growing). The direct arms build their own non-loopback candidate on a `dummy` interface and pin both ends with `--udp-candidate` on a fixed port, so a relay reading means the direct path broke and never that discovery was slow. `upgrade` is the arm that found S-2 (a live relay→direct upgrade was never reported, so `current_path` lied for the rest of the session); `upgrade-late` holds the provider back `SECLEAK_LATE` seconds (90) so the backoff has climbed, and asserts the WORST-CASE relay dwell (<= 75 s) instead of the best case — it red-checks S-8, which lowered the cap from 256 s; `SECLEAK_CONNS`, `SECLEAK_PHASES` and `SECLEAK_KEEP=1` tune it |
| `staging/` | see `staging/README.md` | the **whole staging campaign**, 41 scripts, one reference compose, coordinate-free: `provision.sh` bootstraps both remote hosts from one `env.sh`, then `vm/`, `ws/`, `srv/` and `res/` hold the harnesses that produced `docs/campagna-2026-09-13/evidenze/VHOST_STAGING_EVIDENCE_2026-09-10_DEV_RESULT.md` |
