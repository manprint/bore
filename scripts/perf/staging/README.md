...
| `pub/udp_pool_recycle_fix.sh` | does the P-14 correction hold on a REAL path, against a REAL server? | the SAME two arms with one thing changed — the server is the corrected binary, on the TEST VM, with this workstation as the client so the carrier crosses the WAN. Asserts the shipped binary is the one it built (checksum + version string on both ends) before it measures anything, and prints the pre-correction result beside its own, because an "after" with no "before" does not demonstrate a correction (§43). **Never run beside a jump stage** — it uses the same VM ports (7845/7847). Zero bytes |
