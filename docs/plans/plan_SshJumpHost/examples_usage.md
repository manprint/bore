...
> **Stato:** contratto operativo aggiornato alla fase 4. `sshjhost`, `jump/`,
> `BORE_SSH_JUMP_BASE_DOMAIN` e direct QUIC nativo con fallback TCP sono
> implementati. Nessuna nuova variabile/porta QUIC è stata aggiunta.
>
...
|---|---|---|---|
| client nativo | `bore sshjhost ...` | `BORE_SECRET` attuale | QUIC server-direct opzionale; TCP caldo come fallback |
...
| `443/udp` | `443/udp` | endpoint QUIC condiviso vhost/public/sshjhost |
...
- mantenere `BORE_VHOST_QUIC_PORT=443`: il nome è storico; vhost bare, tunnel
  public (`port:<N>`) e `sshjhost` (`jump:<alias>`) condividono lo stesso accept loop;
...
supportano il path direct `jump:<alias>`.
...
OpenSSH operatore→gateway resta TCP/443. Sulla gamba bore server→provider il
client apre quattro connessioni QUIC indipendenti verso UDP 443, senza
STUN/hole-punch; una sessione usa un solo stream bidi. I quattro carrier TCP
restano connessi. Se QUIC manca o fallisce, la stessa sessione usa subito TCP;
il client rinnova poi solo le connessioni direct mancanti.
...
| `sshjhost --udp` usa TCP | verificare `bore server --udp`, mapping `443/udp`, `BORE_VHOST_QUIC_PORT=443`; il fallback TCP è intenzionale se QUIC non è disponibile |
