...
- **Change:**
  ...
  3. `direct-tcpip` destination parsing: host `"<id>"` or `"secret/<id>"` (port is ignored — a real OpenSSH `-L` client can never send a literal 0, unlike `-R`) ⇒ secret consumer target (used in Phase 5.3); anything else ⇒ channel open failure with message.
