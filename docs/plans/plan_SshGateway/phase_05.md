...
- **Change:** on name collision at registration:
  ...
  - **Implementation note (found during e2e, not anticipated by the plan text above):** the authoritative decision (`apply_takeover`) has to live inside the finalize task, AFTER `await_params` — but `tcpip_forward_vhost`/`tcpip_forward_secret` already reply `Ok(true)` to the SSH `tcpip-forward` global request synchronously, before that task ever runs. A `Reject` decided ~500ms later inside the task can only ever reach the client via `queue_message`, which for a `-N` session (no channel, the common case) is NEVER delivered — so `ExitOnForwardFailure=yes` (T-SSH-TAKE2) would never fire. Fix: a non-mutating `peek_takeover` pre-check runs SYNCHRONOUSLY, before spawning, and returns `Ok(false)` immediately on `Reject`; the spawned task's own `apply_takeover` call remains the sole authoritative mutation (handles the real eviction/insert, and correctly re-decides in the rare case the peek and the authoritative decision disagree due to the residual race between them — same accepted scope as the registry's pre-5.4 vacant-insert race).
- **Unit tests:** `takeover_decision_table` — pure function `(incumbent: Option<(Identity, IsSsh)>, newcomer: Identity) -> Decision` covering: free ⇒ insert; same-id ssh ⇒ evict; diff-id ⇒ reject; native incumbent ⇒ reject. Plus `apply_takeover_vacant_proceeds_without_touching_owners` / `apply_takeover_rejects_native_incumbent` (a full evict-path unit test needs a real `russh::server::Handle`, whose fields are `pub(crate)` to `russh` and so cannot be constructed from this crate without `unsafe` — forbidden crate-wide; that path is covered by the e2e tests instead).
- **e2e tests:**
  **T-SSH-TAKE1** — session A (key K) registers vhost `mysub` backed by service S1; session B (same key K) registers `mysub` backed by S2 ⇒ B succeeds, curl now returns S2's body, A's session receives the eviction message (or exits); no window where curl fails hard for more than the switchover (poll tolerance 2 s). Implemented as `t_ssh_take1_same_identity_vhost_takeover`.
  **T-SSH-TAKE2** — session C (different key) tries `mysub` ⇒ rejected (`ExitOnForwardFailure=yes` makes ssh exit non-zero), B's tunnel still serves S2. Implemented as `t_ssh_take2_different_identity_rejected`.
- **Done:** gates green; T-SSH-TAKE1/2 pass; Opus sign-off on the lock discipline recorded in the PR/commit description — reviewed 2026-07-04, no correctness bugs found: the two-step (decide+remove under the DashMap entry lock, teardown after releasing it) plus the guards' token/`Arc::ptr_eq`-checked `remove_if` correctly prevent a stale evicted guard from clobbering a winner's fresh entry, and `Weak<ConnState>` never keeps a disconnected connection's own `Drop`-driven teardown alive.
