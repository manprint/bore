...
| `secret_leak_hunt.sh` | any Linux host, **no root, no deployment** | `all` (default) \| `churn` \| `churn-relay` \| `churn-direct` \| `stall` \| `stall-direct` \| `reap` \| `upgrade` \| `udpbuf`. The SECRET-tunnel resource gate: descriptors, resident memory, wedges, reaping and socket buffers, for a server + `bore local --tcp-secret-id` + `bore proxy` triple inside a rootless netns (`unshare -rn`). Every churn verdict is a RATE — four identical phases of 200 connections, the LAST phase asserted — because a single before/after pair cannot tell a leak from allocator warm-up (it accused the client of 16 KiB per connection when the truth was an arena that stopped growing). The direct arms build their own non-loopback candidate on a `dummy` interface and pin both ends with `--udp-candidate` on a fixed port, so a relay reading means the direct path broke and never that discovery was slow. `upgrade` is the arm that found S-2 (a live relay→direct upgrade was never reported, so `current_path` lied for the rest of the session); `SECLEAK_CONNS`, `SECLEAK_PHASES` and `SECLEAK_KEEP=1` tune it |
