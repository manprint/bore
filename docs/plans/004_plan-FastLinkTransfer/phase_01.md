...
- **Unit tests:** `mod tests` in `pump.rs` (G-U0), `tokio::io::duplex(1 MiB)` per entrambi i lati, runtime `flavor = "multi_thread", worker_threads = 2` dove indicato:
  ...
  - `pump_coalesces_small_reads` (rev 3) — uploader mock che restituisce ≤ 1 KiB per `poll_read` sempre Ready, 4 MiB, buffer 256 KiB: byte identici e messaggi a W ≤ 20 (hook `#[cfg(test)] on_chunk`); red-check senza coalescenza (4096 messaggi).
