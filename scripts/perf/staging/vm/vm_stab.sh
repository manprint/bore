...
cleanup(){ for p in "${KIDS[@]:-}"; do kill -CONT "$p" 2>/dev/null; kill -9 "$p" 2>/dev/null; done
           sudo -n tc qdisc del dev "$(ip route get $SRV | awk '{print $5;exit}')" root 2>/dev/null; }
...
NETEM(){ local if_=$(ip route get $SRV | awk '{print $5;exit}'); sudo -n "$@" dev "$if_"; }
...
echo "  serving: $(curl -s -o /dev/null -m 20 -w 'http=%{http_code}' https://$L.$GW/ping)"
...
if up "$L" --carriers 1; then
  ...
  ( curl -s -o /dev/null --limit-rate 5M -m 60 -w '  client: code=%{http_code} got=%{size_download} time=%{time_total} exit-on-next-line\n' \
      "https://$L.$GW/stream/1073741824"; echo "  curl exit=$?" ) & C=$!
...
if present "$L"; then
  echo "  origin alive:            $(curl -s -o /dev/null -m 20 -w 'http=%{http_code} t=%{time_total}' https://$L.$GW/ping)"
  kill -9 $O2 2>/dev/null; sleep 2
  echo "  origin dead, tunnel up:  $(curl -s -o /dev/null -m 25 -w 'http=%{http_code} t=%{time_total}' https://$L.$GW/ping)"
  echo "  second request:          $(curl -s -o /dev/null -m 25 -w 'http=%{http_code} t=%{time_total}' https://$L.$GW/ping)"
  ...
  O3=$!; KIDS+=("$O3")
  for i in $(seq 40); do curl -fsS -m 2 -o /dev/null http://127.0.0.1:5053/ping 2>/dev/null && break; sleep 0.25; done
  echo "  origin back, same tunnel: $(curl -s -o /dev/null -m 25 -w 'http=%{http_code} t=%{time_total}' https://$L.$GW/ping)"
...
if present "$L2"; then
  echo "  origin closed (port 1):  $(curl -s -o /dev/null -m 25 -w 'http=%{http_code} t=%{time_total}' https://$L2.$GW/ping)"
  kill -9 $P2 2>/dev/null; wait_gone "$L2" 20
else echo "  L2 did not register"; fi
echo "  unknown subdomain:       $(curl -s -o /dev/null -m 20 -w 'http=%{http_code} t=%{time_total}' https://doesnotexist$(date +%s).$GW/ping)"
;;&
...
if up "$L" --carriers 1 --udp; then
  ...
  curl -fsS -o /dev/null -m 20 "https://$L.$GW/100k"
  d1=$(adm vhost | jq -r --arg l "$L" '.[]|select(.subdomain==$l)|.direct_stream_opens')
  echo "  direct before: opens $d0 -> $d1"
  echo "  throughput before: $(curl -s -o /dev/null -m 30 -w '%{speed_download}' https://$L.$GW/stream/524288000 | awk '{printf "%.2f MB/s", $1/1048576}')"
  ...
  NETEM tc filter add protocol ip parent 1:0 prio 1 u32 match ip dst $SRV/32 match ip protocol 17 0xff flowid 1:3 2>/dev/null \
    && echo "  UDP to server blackholed" || echo "  NETEM FAILED"
  sleep 3
  echo "  request during blackhole: $(curl -s -o /dev/null -m 30 -w 'http=%{http_code} time=%{time_total}' https://$L.$GW/ping)"
  echo "  throughput during:  $(curl -s -o /dev/null -m 40 -w '%{speed_download}' https://$L.$GW/stream/524288000 | awk '{printf "%.2f MB/s", $1/1048576}')"
  ...
  sleep 5
  echo "  throughput after clear: $(curl -s -o /dev/null -m 40 -w '%{speed_download}' https://$L.$GW/stream/524288000 | awk '{printf "%.2f MB/s", $1/1048576}')"
...
ok=0; for L in "${LS[@]}"; do [ "$(curl -s -o /dev/null -m 20 -w '%{http_code}' https://$L.$GW/ping)" = 200 ] && ok=$((ok+1)); done
...
for mode in "" "--udp"; do
  ...
  if up "$L" --carriers 1 $mode; then
    ...
    for n in 16 64 256 512; do
      HOLD=()
      for i in $(seq $n); do curl -s -o /dev/null --limit-rate 20k -m 40 "https://$L.$GW/stream/1048576" & HOLD+=("$!"); done
      sleep 6
...
fresh_req=$(curl -s -o /dev/null -m 15 -w 'http=%{http_code} t=%{time_total}' https://$L.$GW/1k)"
...
for mode in "--udp" ""; do
  ...
  if up "$L" --carriers 1 $mode; then
    ...
    for n in 4 8 16 24 32; do
      SLOW=()
      for i in $(seq $n); do curl -s -o /dev/null --limit-rate 100k -m 120 "https://$L.$GW/stream/536870912" & SLOW+=("$!"); done
      sleep 8
      f1=$(curl -s -o /dev/null -m 15 -w '%{http_code}/%{time_total}' https://$L.$GW/1k)
      f2=$(curl -s -o /dev/null -m 15 -w '%{http_code}/%{time_total}' https://$L.$GW/1k)
