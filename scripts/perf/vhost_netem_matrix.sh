...
SRV=${BORE_SERVER_IP}; GW=${BORE_HOST}
KIDS=()
...
run_pair(){ # <human condition label>
  ...
  for mode in "" "--udp"; do
    ...
    if up "$l" --carriers 1 $mode; then
      ...
      # Sample direct_stream_opens BEFORE and AFTER the measurement: at high RTT
      # the QUIC handshake can still be in flight during warmup, so a single
      # pre-check mislabels a tunnel that goes direct a moment later.
      local d0 d1 out path
      d0=$(dso "$l")
      out=$(meas "$l")
      d1=$(dso "$l")
      if [ "${d1:-0}" -gt "${d0:-0}" ]; then path=direct-quic
      elif [ "${d0:-0}" -gt 0 ]; then path="direct-quic(no new opens)"
      else path=relay-tcp; fi
      echo "    $tag (proven path=$path, opens $d0->$d1): $out"
