...
| 4 — Non-VPN Windows parity | phase_05.md | `IN_PROGRESS` | Added `windows-vpn-build` job to `.github/workflows/ci.yml` (`runs-on: windows-latest`, mirrors the existing `macos-vpn-build` shape): `cargo build --features vpn`, `cargo clippy --features vpn --all-targets -- -D warnings`, `cargo test --features vpn` — the last one runs EVERY integration test file (`tests/*.rs`), not just VPN, on a real Windows host. Per D10 this tier is hosted-non-admin, which is exactly right for non-VPN parity (no TUN/firewall/elevation needed for public/secret/vhost/server/transfer/test-udp). Verified first that none of the 17 non-transfer test files have any Unix-only dependency (no `Command::new` of an external binary, no `/tmp`, no `UnixListener`/`AF_UNIX`); the 2 transfer test files already have Windows-specific `cfg(windows)` branches and already pass on `windows-latest` via the pre-existing `transfer-paths` job. Also added `windows` to `ci.yml`'s push-trigger branch list (mirrors the existing `macos` entry) since `ci.yml` previously never ran on this branch at all. Awaiting first real run result — see Tests table once it reports. |
