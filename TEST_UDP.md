...
metriche QUIC del path diretto (`rtt`, `cwnd`, loss, MTU, PLPMTUD, flow-control), più lo skew upload/download, il preset consigliato e i valori del tuning usato. Se il
...
tuning bulk, i primi knob sono in `src/shared.rs` (consumati da `src/holepunch.rs`): 16 MiB per
