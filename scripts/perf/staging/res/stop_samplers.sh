...
# ONE prefix, shared with start_samplers.sh. The two used to disagree
# (`wsres` remote, `pres` here) and this step quietly collected nothing.
REMOTE_PREFIX="${SAMPLER_PREFIX:-pres}"
for x in stat proc; do
    [ -n "$BORE_SRV" ] && vmcp "$BORE_SRV_USER@$BORE_SRV:~/$REMOTE_PREFIX.$x" "$OUT/pres_srv.$x" 2>/dev/null
    vmcp "$BORE_VM_USER@$BORE_VM:~/$REMOTE_PREFIX.$x" "$OUT/pres_vm.$x" 2>/dev/null
done
